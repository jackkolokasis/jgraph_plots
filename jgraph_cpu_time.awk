...
#
# For multiline graph titles, configuration names,
# or bar labels use backslashes as such:
#
# foo\bar\zoo will result in:
#
# foo
# bar
# zoo
...

{
	if(NF == 1){
		...
		mark_label[nr_configs - 1][plot_index] = gensub("\\\\", "\\\\\n", "g", $1)
...
END {
	...
	printf("xaxis min 0 max %.2f no_auto_hash_marks no_auto_hash_labels\n",
	       total_plots + 0.8);
	printf("xaxis hash_labels fontsize 7 linesep 2 rotate 45 hjc\n")
	printf("xaxis draw_hash_labels_at %.3f\n", -1 * (exec_time_max * 10 / 100))
	...
	for(i = 0; i < nr_configs; i++){
		...
		for(j = 0; j < grouped_bars[i]; j++){
			...
			printf("xaxis hash_label at %.2f : %s\n", plot_index - (0.3 * (grouped_bars[i] - 1)), mark_label[i][j]);
		...
		# Configuration name
		printf("newstring fontsize 9 hjc vjc x %.2f y %.2f : %s\n", (plot_index + plot_index_initial) / 2 - (i ? 0.1 : 0.3),
		       exec_time_max + exec_time_max / 20, hash_label[i]);
		plot_index += 0.8;
	}
